...
def calculate_minimum_performers(groups_ideal: int) -> int:
    ...
    - At least 2 performers eliminated in preselection
    ...
    Examples:
        ...
        6  # 2 pools * 2 performers + 2 eliminated = 6 minimum

        >>> calculate_minimum_performers(3)
        8  # 3 pools * 2 performers + 2 eliminated = 8 minimum

        >>> calculate_minimum_performers(4)
        10  # 4 pools * 2 performers + 2 eliminated = 10 minimum
    """
    if groups_ideal < 1:
        raise ValueError("groups_ideal must be at least 1")

    return (groups_ideal * 2) + 2
...
def calculate_pool_capacity(
    ...
    Strategy:
    - Eliminate approximately 20-25% of registered performers
    - Ensure at least 2 eliminated (preselection mandatory)
    ...
    Examples:
        >>> calculate_pool_capacity(8, 2)
        (6, 2)  # Eliminate 2, create 2 pools of 3 each

        >>> calculate_pool_capacity(10, 2)
        (8, 2)  # Eliminate 2, create 2 pools of 4 each

        >>> calculate_pool_capacity(20, 2)
        (16, 4)  # Eliminate 4 (20%), create 2 pools of 8 each

        >>> calculate_pool_capacity(6, 2)
        (4, 2)  # Minimum case: eliminate 2, create 2 pools of 2 each
    """
    ...
    # Calculate elimination target (~20-25%, minimum 2)
    elimination_target = max(2, int(registered_performers * 0.25))
...
def calculate_minimum_for_category(
    ...
    Returns:
        ...
        - min_eliminated: Minimum eliminated in preselection (always 2)
